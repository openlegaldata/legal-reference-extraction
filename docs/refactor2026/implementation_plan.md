...
| 6 | **H3** — full `Ref` / `RefMarker` deletion | Stream H | **Partial 2026-04-21** — dead helpers removed from `models.py` (`total_ordering`, `get_case_repr`, `get_law_repr`, `get_end_position`, `get_length`; `__lt__` kept for `assert_refs` sort, `get_start_position` kept for `compat.to_ref_marker_string`).  Full deletion still blocked by three load-bearing consumers: (a) `law.py` + `case.py` emit `RefMarker`/`Ref` as their primary output (~1 000 LOC to rewrite), (b) `refex.compat` exposes `citations_to_ref_markers` / `to_ref_marker_string` by design for Open Legal Data's legacy pipeline, (c) 180+ test assertions go through `assert_refs` on `list[Ref]`.  This is the refactor's final major-version cleanup item — deferring to a follow-on ticket. |
