...
| 4 | ~~**E2** — `default_unit` column on `law_book_codes.txt`~~ | Stream E | **Landed 2026-04-21.**  Parser accepts optional tab-separated `<unit>` column (`article`/`paragraph`).  Mixin exposes `get_unit_hint(code)`; `_law_markers_to_citations` consults it and overrides the text-prefix heuristic when present.  Curated 23 high-confidence annotations: `GG`/`EUV` = article; `BGB`/`HGB`/`StGB`/`StPO`/`ZPO`/`VwGO`/`VwVfG`/`AsylG`/`BNatSchG`/`BauGB`/`SGB 1-12`/`SGB II-XII` = paragraph. Remaining ~1 920 codes stay heuristic-driven. Validation F1 unchanged (unit is not in the metric); data-file semantics can now override wrong matches (e.g. `§ 3 GG` → unit=article). |
| 5 | ~~**G (transformer) Hub push**~~ | Stream G | **Landed 2026-04-21** at [`openlegaldata/legal-reference-extraction-base-de`](https://huggingface.co/openlegaldata/legal-reference-extraction-base-de) (private, CC BY-NC 4.0).  Uploaded weights + tokenizer + EuroBERT custom modeling files and a full model card (evaluation block cites an "unreleased benchmark" with per-engine F1 tables).  Training checkpoints / optimizer state excluded.  `refex.engines.transformer.DEFAULT_MODEL` now points at this repo so `TransformerExtractor()` with no args loads it by default. |
| 6 | **H3** — full `Ref` / `RefMarker` deletion | Stream H | **Partial 2026-04-21** — two passes of deletion landed: first pass removed dead helpers (`total_ordering`, `get_case_repr`, `get_law_repr`, `get_end_position`, `get_length`); second pass removed the deprecated inline-marker surface (`to_ref_marker_string`, `RefMarker.replace_content`, `_MARKER_*_FORMAT`, `get_start_position`) + the dead `BaseRef.sentence` attribute.  `Ref`, `RefMarker`, and `citations_to_ref_markers` remain because (a) `law.py` + `case.py` emit `RefMarker`/`Ref` as their primary output (~1 000 LOC to rewrite to emit `LawCitation`/`CaseCitation` directly), and (b) `conftest.py::assert_refs` + 180+ fixtures still compare `list[Ref]`.  The regex-engine rewrite is the last step and is a dedicated major-version effort. |
...
| 8 | ~~**Aho–Corasick court-name index**~~ | post-regex optimization | **Closed 2026-04-21: measured and rejected.**  Pure-Python equivalent (one-pass `re.finditer` + bisect spatial join) regressed throughput −35.6 %; Python's C `re` beats the single-pass approach on typical docs because low per-char cost outweighs saved `finditer` setup.  A C-backed AC dep is out of scope.  See `optimization_log.md` §"§7/#8". |
| 9 | ~~**Per-`(doc_id, fn_span)` court cache**~~ | post-regex optimization | **Closed 2026-04-21: measured and rejected.**  Same-file-number recurrence is 16.9 % (measured), but court resolution is position-dependent — cache-first regresses span F1, fresh-first with cache fallback regresses court accuracy *and* throughput.  See `optimization_log.md` §"§7/#9". |
| 10 | ~~**Interval-based marker masking**~~ | post-regex optimization | **Landed 2026-04-21.**  New `_apply_mask_intervals(content, [(s, e), …])` helper in `law.py`; each extraction phase now collects its match spans and applies the mask in one O(len(content)) pass at the end of the phase instead of O(N × len(content)) per-marker `content = marker.replace_content_with_mask(content)` calls.  Equivalent semantics (per-iteration masks didn't affect the already-created `finditer` anyway).  Validation F1 unchanged; +1 % throughput on the validation split. |
